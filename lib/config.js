...
/**
 * Current Configuration is a dynamic variable that works with one stack (not multiple
 * fibers).
 *You can dynamically set the CurrentConfig value for a given position
...
function withValidation (configValue)
{
   ...
   if (engines.client && !_.contains(supportedEngines.client, engines.client))
    ...
    engines.client = defaultValue(engines.client, 'blaze');
