...
Command.create({
  ...
  if (this.isFile('.iron/config.json')) {
    try {
      var oldConfig = JSON.parse(fs.readFileSync('.iron/config.json'));
  ...
  fs.unlinkSync('app/.iron/config.json');
  fs.rmdirSync('app/.iron');
  ...
  Maka.findCommand('init').invoke([], initOptions);
