...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    Maka.findGenerator('methods').invoke(['methodName'], {cwd: projectDirectory, where: 'both'});
    Maka.findGenerator('methods').invoke(['methodName'], {cwd: projectDirectory, where: 'server'});

    if (!_.has(opts, 'skip-flow-router')) {
      // install the iron router package
      // kadira:flow-router
      // kadira:blaze-layout
      self.installMeteorPackage('kadira:flow-router', {cwd: appDirectory});
      self.installMeteorPackage('kadira:blaze-layout', {cwd: appDirectory});
    }

    if (!_.has(opts, 'skip-jasmine')) {
      // install the jasmine driver package and html/console reporter
      self.installMeteorPackage('sanjo:jasmine velocity:html-reporter velocity:console-reporter', {cwd: appDirectory});
    }
    ...
    if (config.template.css) {
      ...
      Maka.findGenerator('stylesheet').invoke(['main'], {cwd: projectDirectory});
    }
    ...
    // Inject the template loading imports at the beginning of the template JS files
    _.each([
      'imports/ui/templates/layouts/master-layouts/master-layout.js',
      'imports/ui/templates/pages/default.js'
    ], function(jsPath) {

      var config = CurrentConfig.get();
      basename = path.basename(jsPath, '.js');
      jsPath = path.join(appDirectory, jsPath);


      self.injectAtBeginningOfFile(
        jsPath,
        "import './" + basename + "." + config.engines.html + "';"
      );
    });
