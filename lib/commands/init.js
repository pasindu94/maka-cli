...
Command.create({
  ...
  var config = {
    engines: {
      ...
      css: opts.css || 'css',
      api: opts.api || 'rest',
      client: opts.client || 'blaze',
      tests: opts.test || 'jasmine'
    },
    template: {
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    Maka.findGenerator('template').invoke(['MasterLayout', '--layout'], {cwd: projectDirectory, root: true});
    Maka.findGenerator('template').invoke(['home'], {cwd: projectDirectory, root: true});
    Maka.findGenerator('template').invoke(['NoteFound'], {cwd: projectDirectory, root: true});

    if (!_.has(opts, 'skip-flow-router') && opts.client === 'blaze') {
    ...
    if (opts.client === 'react') {
        self.installNpmPackage('react react-dom react-router', {cwd: appDirectory});
    }
