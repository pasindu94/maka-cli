...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    if (!_.has(opts, 'skip-validated-methods')) {
      self.installMeteorPackage('mdg:validated-method aldeed:collection2@2.10.0', {cwd: appDirectory});
    }
    ...
    if (opts.client === 'react') {
        ...
        self.installNpmPackage('react react-dom react-router react-addons-pure-render-mixin', {cwd: appDirectory});
    }
