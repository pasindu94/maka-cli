...
module.exports.rewriteDestinationPathForEngine = function rewriteDestinationPathForEngine(/* path parts */) {
    ...
    if (destpath.indexOf('startup/lib') > -1 || destpath.indexOf('startup/server') > -1 || destpath.indexOf('app/server') > -1 || destpath.indexOf('app/lib') > -1) {
        return destpath;
    }
...
module.exports.isTemplateForEngine = function isTemplateForEngine(/* path parts */) {
    ...
    var fileExists = this.isFile(srcpath + '.' + configuredEngine);
    if (!fileExists && srcpath.indexOf(configuredEngine) === -1) {
        configuredEngine = 'js';
    }
...
module.exports.templateContent = function templateContent(srcpath, context) {
    ...
    if (!this.isFile(tmplpath)) {
        var pathArray = tmplpath.split('.');
        tmplpath = pathArray.slice(0, pathArray.length -1).join('.');
        if (!this.isFile(tmplpath)) {
